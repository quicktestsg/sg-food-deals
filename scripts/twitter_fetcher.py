#!/usr/bin/env python3
"""
Twitter/X Deal Fetcher for SG Food Deals.
...
SOURCES = [
    {
        "label": "SINGPromos",
        "query": "from:singpromos",
        "color": "#f59e0b",
        "default_type": "deal",
    },
    {
        "label": "GreatDealsSG",
        "query": "from:GreatDealsSG",
        "color": "#ef4444",
        "default_type": "deal",
    },
]

# ─── Keyword filters (same as RSS fetcher) ───
INCLUDE_KEYWORDS = [
    "1-for-1", "1 for 1", "1for1",
    "promo", "promotion", "deal", "deals",
    "discount", "sale", "offer", "offers",
    "free", "giveaway", "voucher", "coupon",
    "$", "off", "up to", "flash",
    "优惠", "促销", "打折", "折扣", "特价", "免费", "买一送一",
]
...
def detect_deal_type(text):
    text_lower = text.lower()
    if any(kw in text_lower for kw in ["1-for-1", "1 for 1", "1for1", "买一送一", "半价"]):
        return "1fl"
